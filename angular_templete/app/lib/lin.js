...
var F = {
	...
	addcss: function(d, css) {
		var css = css || this.css,
			cs = d.className.replace(/\s{2,}/g, ' ').split(' '),
			c = true;
...
(function(w) {
	...
	for (var i = 0; i < sp.length; i++) {
		if (sp[i].getAttribute('src') && sp[i].getAttribute('src').indexOf('lin.js') > -1) {
		...
		this.cj = function() {
			...
			if (typeof(ob.css) == 'string') {
				...
				if (ob.css == 'ui_confirm') {
					...
					this.G_d.innerHTML = '<p class="ui_p1">' + ob.s + '</p><p class="ui_p2"><textarea rows="6" class="ui_text" req="true" ts="输入内容不能为空"></textarea></p><p class="ui_p"><input type="button" class="ui_button" value="提交" /><input type="button" class="ui_button ui_button_r" value="取消" /></p>';
					var ad = this.G_d.children[2];
					ad.children[0].G_d = this.G_d;
					ad.children[0].yc = this.yc;
					ad.children[0].f = ob.f;
					this.G_d.style.backgroundColor = '#FFFFFF';
					this.G_d.style.width = "70%";
					if (m) {
						ad.children[0].c = m
					};
					ad.children[0].onclick = function() {
